...
const Likes = () => {
    ...
    // Likes item from DB: { tmdbId, title, posterPath }
    // MovieCard expects: { id, title, poster_path, vote_average }
    const likedMovies = library?.likes?.map(item => ({
        id: item.tmdbId,
        title: item.title,
    ...
    if (likedMovies.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-4">
                ...
                <h2 className="text-2xl font-bold">No liked movies yet</h2>
    ...
    return (
        <div className="space-y-8 pb-20">
            <div className="flex items-end gap-3 mb-6">
                <h2 className="text-3xl font-bold tracking-tight">Liked Movies</h2>
                <span className="text-gray-500 mb-1.5 text-sm">{likedMovies.length} movies</span>
            </div>
